...
import com.userservice.exception.UserDataException;
import com.userservice.repository.UserDataRepository;
import com.userservice.utils.EmailService;
import com.userservice.utils.Token;
...
public class UserService implements IUserDataService {
	...
	public UserData createNewUser(UserDataDTO userdto) {
		...
		if (checkEmailId.isPresent()) {
			...
			UserData userdata = new UserData(userdto.getFullName(), userdto.getPhoneNumber(), userdto.getEmailId(),
					password);
	...
	public UserData getUserById(String tokenId) {
		UUID token = jwtToken.decodeJWT(tokenId);
		UserData userData = userdatarepo.findById(token)
				.orElseThrow(() -> new UserDataException("User is not PRESESNT"));
	...
	public UserData getUser(String tokenId) {
		UUID token = jwtToken.decodeJWT(tokenId);
	...
	public String resetPassword(ResetPasswordDto resetPasswordDto, String token) {
		...
		UserData userdata = userdatarepo.findById(userId).orElseThrow(() -> new UserDataException("User not found"));
